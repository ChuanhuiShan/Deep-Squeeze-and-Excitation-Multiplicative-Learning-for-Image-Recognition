...
# #######################################################
# glob_pool1 = tf.reduce_mean(h_conv1, [1, 2], name='global_pool', keep_dims=True) # Global pooling
# glob_pool_flat1 = tf.reshape(glob_pool1, [-1, 32])   

# Se_W_fc1 = weight_variable([32, 8])
# Se_fc1 = tf.nn.relu(tf.matmul(glob_pool_flat1, Se_W_fc1))

# Se_W_fc2 = weight_variable([8, 32])
# Se_fc2 = tf.sigmoid(tf.matmul(Se_fc1, Se_W_fc2))

# Se_fc2_tensor1 = tf.reshape(Se_fc2, [-1, 1, 1, 32])  
# ##################################################################

# h_conv1 = tf.multiply(h_conv1, Se_fc2_tensor1)
...
# #######################################################
# glob_pool2 = tf.reduce_mean(h_conv2, [1, 2], name='global_pool', keep_dims=True) # Global pooling
# glob_pool_flat2 = tf.reshape(glob_pool2, [-1, 64])   

# Se_W_fc3 = weight_variable([64, 16])
# Se_fc3 = tf.nn.relu(tf.matmul(glob_pool_flat2, Se_W_fc3))

# Se_W_fc4 = weight_variable([16, 64])
# Se_fc4 = tf.sigmoid(tf.matmul(Se_fc3, Se_W_fc4))

# Se_fc2_tensor2 = tf.reshape(Se_fc4, [-1, 1, 1, 64])  
# ##################################################################

# h_conv2 = tf.multiply(h_conv2, Se_fc2_tensor2)
